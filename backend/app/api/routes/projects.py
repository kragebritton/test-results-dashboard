...
@router.get("/projects")
async def list_projects(storage: ProjectStorageService = Depends(get_storage_service)) -> list[dict[str, object]]:
    return storage.list_projects()


@router.get("/overview")
async def project_overview(storage: ProjectStorageService = Depends(get_storage_service)) -> list[dict[str, object]]:
    return storage.project_overview()


@router.get("/projects/{project}")
async def project_details(project: str, storage: ProjectStorageService = Depends(get_storage_service)) -> dict[str, object]:
    return storage.project_details(project)
...
async def upload_results(
    ...
    background_tasks.add_task(storage.process_upload, project, file_content, build_id)
...
async def serve_report(
    project: str, path: str = "index.html", storage: ProjectStorageService = Depends(get_storage_service)
) -> FileResponse:
    safe_path = storage.get_report_path(project, path)
